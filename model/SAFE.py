...
class SAFE(nn.Module):
    ...
    def __init__(self, feature_sizes, seed, num_categories, num_continuous, config=None,batch_size=1000, embedding_size=4, hidden_dims=[32, 32], num_classes=3, dropout=[0.5, 0.5], 
        ...
        # +512
        self.out1 = nn.Linear(8*32+32+512, 2) #num_categories*32+num_continuous+512.        532
        # +512
        self.out2 = create_cosNorm_model(in_dims=8*32+32+512, out_dims=2) #
    ...
    def fit(self, loader_train, loader_val, loader_test,loader_train_test, optimizer, config, year, epochs=100, verbose=False, print_every=100):
        ...
        for e in range(epochs):  
            ...
            for t, (xi, xv, y, img_cnn, img_gcn, anchor_xi, anchor_xv, anchor_cnn, anchor_gnn) in enumerate(loader_train):
                ...
                if verbose and t % print_every == 0:
                    ...
                    print('--------------------train---------------------')
                    f_train, preds_train = self.check_accuracy(loader_train_test, model, f'Epoch{e} Iteration{t}')
                    print('--------------------valid---------------------')
                    f_valid, preds_valid = self.check_accuracy(loader_val, model, f'Epoch{e} Iteration{t}')
                    print('--------------------test---------------------')
                    f_test, preds_test = self.check_accuracy(loader_test, model, f'Epoch{e} Iteration{t}')
                    print()

                    if f_test >= max_f1:
                        max_f1 = f_test
                        out_model_pth = config['result_dir'] + f'/best_{year}_{self.seed}.pt'
                        self.save(model, out_model_pth)
                        np.savetxt(f'/results/results_train_{year}_{self.seed}.csv', preds_train, fmt='%d', delimiter=',')
                        np.savetxt(f'/results/results_valid_{year}_{self.seed}.csv', preds_valid, fmt='%d', delimiter=',')
                        np.savetxt(f'/results/results_test_{year}_{self.seed}.csv', preds_test, fmt='%d', delimiter=',')

        dataset_mode = config['dataset_mode']

        import datetime
        today = str(datetime.date.today()).replace('-', '')
        with open(f'./log/ours{today}_{year}_{dataset_mode}.txt', 'a') as f:
            f.write(str(max_f1)+'\n')
