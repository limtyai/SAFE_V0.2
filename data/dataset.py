...
class CategoryDictGenerator:
    ...
    def build(self, datafile, categorial_features, cutoff=0):
        ...
        for j in range(0, self.num_feature):
            spatital_data = torch.zeros((5, self.wall_lat, self.wall_lon)) #clusterid
...
class ContinuousFeatureGenerator:
    ...
    def build(self, datafile, continous_features):
        ...
        self.final_spatial_data = torch.zeros((length, self.num_feature, 2*self.bias, 2*self.bias))
        for j in range(0, self.num_feature):
            spatital_data = torch.zeros((5, self.wall_lat, self.wall_lon)) #clusterid，3=》5
...
class CriteoDataset(Dataset):
    ...
    def __init__(self, root, train=True, mode='train', year=2019):
        ...
        self.file = config['data_dir'] + f'/{year}_{dataset_mode}_{mode}.csv'
        # for cnn
